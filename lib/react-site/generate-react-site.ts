...
export function generateReactSiteFiles(projectName: string, slug: string, raw: unknown): GeneratedFile[] {
  ...
  // La multimedia ya se subio a S3 y el contenido apunta a esas URLs (ver la
  // ruta de creacion): NO se hornea ningun binario en el repo.
  const content = saved.content && typeof saved.content === "object" ? saved.content : {};
  // Aplica las fuentes efectivas (override del doc o default del template).
